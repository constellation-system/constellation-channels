...
where
    ...
    flows: HashMap<Channel::Param, RegistryFlows<F>>
}
...
where
    ...
    /// Map from names to `F::ChannelID`s.
    ids: HashMap<String, F::ChannelID>,
    /// Reverse map from `F::ChannelID`s to names.
    names: Vec<String>,
    ...
    Channel::Param: Clone + Display + Eq + Hash + PartialEq
{
    ...
    #[inline]
    pub fn ids(&self) -> Iter<'_, String, F::ChannelID> {
        self.ids.iter()
    }
    ...
    fn stream(
        ...
        self.registry
            ...
            .map_ok(|session| {
                ...
                let id = StreamID::new(addr.clone(), id.clone(), param.clone());
    ...
    fn create(
        ...
        let ids =
            if !names.is_empty() {
                ...
                registry.ids().map(|(_, id)| id.clone()).collect()
            };
...
impl From<&'_ FarChannelRegistryID> for usize {
    #[inline]
    fn from(val: &FarChannelRegistryID) -> usize {
        val.0
    }
