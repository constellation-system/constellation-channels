...
#[derive(Debug)]
pub enum AcquiredEntryListenError<
    Refresh,
    Flows,
    Start,
    AuthN,
    ...
    where
        ...
        match state {
            ...
            Some(SessionState::Pending { pending }) => {
                ...
                let res = pending.into_auth(authn, flow);
    ...
    Types: FlowsEntryTypes<Flow>
{
    ...
    > {
        let encoder = Types::Encoder::create(encoder_config.clone())
            .map_err(|err| FlowsEntryCodecError::Encoder { err: err })?;
        let decoder = Types::Decoder::create(decoder_config.clone())
            .map_err(|err| FlowsEntryCodecError::Decoder { err: err })?;
    ...
    > {
        ...
        match self.sessions.entry(endpoint.clone()) {
            ...
            Entry::Vacant(ent) => {
                ...
                .map_err(|err| FlowsEntryReqFlowError::GetFlow { err: err })?
                .map_ok(|session| {
                    session
                        .map(|session| {
                            self.session_to_chan(
                        ...
                        Entry::Occupied(mut ent) => {
                                ...
                                })?
                            {
                                ...
                                let chan = self
                                    ...
                                    .map_err(|err| {
                                        SessionListenError::Codec { err: err }
                                    })?;
            ...
            for endpoint in endpoints.drain() {
                    ...
                    self.sessions.entry(endpoint.clone())
                {
                    ...
                    if let Some(session) = session {
                        ...
                        let peer_addr = session.peer_addr().map_err(|err| {
                            SessionListenError::IO { err: err }
                        })?;
                        let chan = self
                            .session_to_chan(
        ...
        let res = match &mut resolver {
                ...
                .map_ok(|(resolved, time)| {
                    ...
                    for (addr, _, _) in resolved {
                        if policy.check_ip(&addr.ip()) {
                            ...
                            if let Some(curr) =
                                tokens.insert(addr.clone(), token)
                            {
                            ...
                            if flows.insert(token, ent).is_some() {
                                error!(target: "acquired-entry",
            ...
            AcquiredResolver::StaticMulti { params } => {
                ...
                for addr in params {
                    ...
                    if let Some(curr) = tokens.insert(addr.clone(), token) {
                        error!(target: "acquired-entry",
                               "entry exists for address {}: token {}",
                               addr, curr.0);
                    }
            ...
            AcquiredResolver::StaticSingle { param } => {
                ...
                if let Some(curr) = tokens.insert(param.clone(), token) {
                    error!(target: "acquired-entry",
                           "entry exists for address {}: token {}",
                           param, curr.0);
                }
    ...
    Types: FarChannelsTypes
{
    ...
    /// # Return Value
    ///
    /// - `(self, Some(when))`: The next refresh or retry occurs at `when`.
    ///
    ...
    where
        ...
        match self.acquired.take().ok_or(ChannelEntryListenError::None)? {
            RetryResult::Success(AcquireState::Pending { state }) => {
                match self.channel.complete_negotiate(state) {
                    ...
                    Ok(NegotiatorResult::Complete(acquired)) => {
                        ...
                        self.handle_acquired(ctx, &mut created, acquired)
    ...
    Types: FarChannelsTypes
{
    ...
    fn listen(
        ...
        RetryResult<(
            Self::StreamIter,
            Self::EndpointIter,
            Option<Vec<(FarChannelID, Option<Vec<Types::ChannelParam>>)>>,
            Option<Instant>
        )>,
        Self::ListenError
    > {
        ...
        let mut updates: Option<
            Vec<(FarChannelID, Option<Vec<Types::ChannelParam>>)>
        > = None;
        ...
        for id in lives {
            match self.channels[id.0].listen(
                ctx,
                |chan, param, endpoint| {
                    sessions.push((endpoint, id.clone(), param.clone(), chan));
                },
                |endpoint, param| {
                    endpoints.push((
                        endpoint.clone(),
            ...
            if !self.channels[id.0].is_shutdown() {
                ...
                if !self.channels[id.0].is_shutdown_safe() {
                    match self.channels[id.0]
                        .listen(
                            ctx,
                            |stream, param, endpoint| match &mut sessions {
                                Some(sessions) => {
                                    sessions.push((
                                        endpoint,
                                        id.clone(),
                        ...
                        .map_err(|err| {
                            FarChannelsShutdownListenError::Listen { err: err }
                        })? {
    ...
    Types: FarChannelsTypes
{
    ...
    fn create(
        ...
        for config in channel_configs {
            ...
            .map_err(|err| FarChannelsCreateError::Entry { err: err })?;
...
fn entry_test<Types, Ctx>(
    ...
    let listen = spawn(move || {
        let addr_config =
            AddrsConfig::new(addr_kinds.to_vec(), server_resolve_config);
        ...
        let mut sessions: Vec<Types::AuthNChan> = Vec::new();
